...
def scrape_yelp_data():
    ...
    for i in range(10):
        ...
        try:
            general_data = soup.find("script", {
                                     "type": "application/json", "data-hypernova-key": "yelpfrontend__5385__yelpfrontend__GondolaSearch__dynamic"})
        except IndexError:
        ...
        for restaurant in general_json_data['legacyProps']['searchAppProps']['searchPageProps']['mainContentComponentsListProps']:
            ...
            if 'bizId' in restaurant.keys():
                restaurant_name = restaurant['searchResultBusiness']['name'].replace(
                    '&amp;', '&').replace('\u2019', "'")
        ...
        try:
            location_data = soup.find("script", {
                                      "type": "application/json", "data-hypernova-key": "yelpfrontend__5385__yelpfrontend__GondolaSearch__dynamic"})
        except IndexError:
            print("Script tag not found")
            continue
        ...
        for restaurant in restaurant_list:
            for maker in location_json_data['legacyProps']['searchAppProps']['searchPageProps']['rightRailProps']['searchMapProps']['mapState']['markers']:
                if 'url' in maker.keys() and maker['url'] in restaurant['businessUrl']:
                    ...
                    restaurant['geohash'] = get_geohash(maker['location']['latitude'], maker['location']['longitude'])
                    print(
                        f"{restaurant['restaurant_name']} - {restaurant['latitude']}, {restaurant['longitude']}")
                    break
    ...
    for restaurant in restaurant_list:
        print(
            f"Scraping data from: {rank} - {restaurant['restaurant_name']} starts >>>")
        ...
        # fetch the json script
        try:
            address_reviews_data = soup.find_all(
        ...
        # format the json string
        address_reviews_json_str = re.sub(
            r"<!--|-->", "", address_reviews_text)
        # json string -> python dict
        ...
        reviews = sorted(
            reviews, key=lambda x: x['reviewRating']['ratingValue'], reverse=True)[:3]
        ...
        print(
            f"Scraping data from: {rank} - {restaurant['restaurant_name']} done   <<<")
        rank += 1
